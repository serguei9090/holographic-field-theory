...
def main():
    ...
    if not os.path.exists(checkpoint_path):
        print(
            "❌ ERROR: No checkpoint file found (neither 'best_chft_checkpoint.pth' nor 'chft_checkpoint.pth')."
        )
    ...
    try:
        ...
        if "pos_rotation_real" in checkpoint["codebook_state_dict"]:
            CONTEXT_LEN = checkpoint["codebook_state_dict"]["pos_rotation_real"].shape[
                0
            ]
        else:
            CONTEXT_LEN = checkpoint["codebook_state_dict"]["pos_weights"].shape[0]
        print(
            f"⚙️ Detected parameters from checkpoint: Dim={DIMENSION}, Context={CONTEXT_LEN}"
        )

        print("⏳ Initializing model architecture...")
        codebook = FHRRPhasorEmbedding(vocab_size, DIMENSION, CONTEXT_LEN).to(device)
        hopfield_mem = ModernHopfieldMemory(DIMENSION).to(device)

        codebook.load_state_dict(checkpoint["codebook_state_dict"], strict=False)
        hopfield_mem.load_state_dict(checkpoint["hopfield_state_dict"], strict=False)
        hopfield_mem.update_keys(codebook)
        print(
            f"✅ Model loaded successfully (saved at Epoch {checkpoint['epoch'] + 1})."
        )
    ...
    while True:
        try:
            ...
            print("-" * 45)
            print(
                f"⏱️ Time taken: {elapsed:.3f}s | Speed: {tok_per_sec:.1f} tokens/second"
            )
